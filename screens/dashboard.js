...
function _buildSesiHTML() {
  ...
  // Runtime (step 2-4) butuh wrap fit-screen tanpa scroll — pre-apply
  // class --runtime di markup awal agar tidak ada FOUC sebelum _rtMount.
  // Step lain (0,1,5,6) = stepper biasa → scroll natural via .dash-scroll-area.
  const isRuntimeStep = step >= 2 && step <= 4;
  const runtimeClass  = isRuntimeStep ? ' ds-stepper-wrap--runtime' : '';

  return `
<div class="ds-wrap ds-stepper-wrap${runtimeClass}">
